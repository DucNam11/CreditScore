...
def plot_percentage_missing_values_for(data, long, larg):
    ...
    plt.title("PROPORTIONS OF VALUES COMPLETED / NOT COMPLETED BY COLUMN",
              fontweight="bold",
              fontsize=TITLE_SIZE, pad=TITLE_PAD)
    ...
    axis.set(ylabel="Columns", xlabel="Percentage of values ​​(%)")
...
def plotBoxPlots(data, long, larg, nb_rows, nb_cols):
    ...
    # Initialize the figure and axes array
    f, axes = plt.subplots(nb_rows, nb_cols, figsize=(long, larg))
    
    # Set the main title
    f.suptitle("QUANTITATIVE VALUES - DISTRIBUTION", fontweight="bold",
               fontsize=TITLE_SIZE, y=TITLE_PAD)

    # Flatten the axes array in case of multiple subplots
    axes = axes.flatten() if nb_rows * nb_cols > 1 else [axes]

    for idx, ind_quant in enumerate(data.columns.tolist()):
        ax = axes[idx]
        ...
        ax.set_xlabel(ind_quant, fontsize=LABEL_SIZE, labelpad=LABEL_PAD, fontweight="bold")
...
def plotDistplotsWithRug(data, long, larg, nb_rows, nb_cols):
    ...
    f.suptitle("DISTRIBUTION OF QUANTITATIVE VALUES", fontweight="bold",
               fontsize=TITLE_SIZE, y=TITLE_PAD)
...
def plot_qualitative_dist(data, nb_rows, nb_cols, long, larg, title):
    ...
    fig.suptitle("DISTRIBUTION OF QUALITATIVE VALUES"+" "+title,
...
def plot_correlation_heatmap(data, long, larg):
    ...
    f.suptitle("CORRELATION MATRIX - SPEARMAN", fontweight="bold",
               fontsize=TITLE_SIZE, y=TITLE_PAD)

    # Update mask argument to use a boolean array with bool
    mask = np.zeros_like(data, dtype=bool)
    
    b = sns.heatmap(data, mask=mask,
                    cmap=sns.diverging_palette(220, 10, as_cmap=True), square=True, ax=ax,
                    annot=data, annot_kws={"fontsize":10}, fmt=".2f")
